...
local function item_count_from_power_usage(power_usage, stack_size)
	...
	return (
		((power_usage - 1000000 / 60) / power_usages[(settings.global["memory-unit-power-usage"]).value])
			^ (1 / 0.35)
		* (stack_size or 1000)
	)
end
...
local function patch_storage(entity)
	...
	local combinator = surface.find_entity(
		"memory-unit-combinator",
	...
	if not combinator_control then
		return
	end

	local signal = combinator_control.get_signal(1)
	if not signal or not signal.signal or signal.signal.type ~= "item" then
		game.print("Unable to recover unit data.")
		return
	end
