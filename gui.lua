...
local function format_energy(energy)
	...
	local suffix = unit_table[mag]

	return suffix and string.format("%.2f", consumption) .. suffix or "A lot!"
end

local function mark_warning(header, bool)
	if bool then
		header.style = "mu_negative_subheader_frame"
	else
...
local function update_gui(gui, fresh_gui)
	...
	if not entity.valid or not powersource.valid then
		return
	end
	...
	--- Memory UI
	memory_frame.memory_containment_flow.memory_containment_strength.value = unit_data.containment_field
		/ containment_field_max
	memory_frame.memory_status_flow.memory_electricity_flow.memory_electricity_label.caption = format_energy(
		powersource.energy
	) .. " / [font=default-semibold][color=255,230,192]" .. format_energy(powersource.electric_buffer_size) .. "[/color][/font]"
	memory_frame.memory_header.memory_header_flow.memory_header_tier_label.caption =
		{ "mod-gui.tier-info", unit_data.energy_tier or "?", 8 }
	if unit_data.energy_tier and unit_data.energy_tier < 8 then
		memory_frame.memory_header.memory_header_flow.memory_header_tier_label.tooltip =
			{ "mod-gui.memory-tab-tier-tooltip", -math.ceil(unit_data.energy_to_next_tier * 100) }
	end

	if unit_data.item then
		update_power_usage(unit_data, count + inventory_count)
	end
	...
	local function states()
		if not last_action or last_action == 0 then
			return { "entity-status.idle" }
		end
		if last_action < 0 then
			return { "entity-status.inserting" }
		end
		if last_action > 0 then
			return { "entity-status.extracting" }
		end
	end
	...
	mc_frame.mc_header.mc_header_flow.mc_header_tier_label.caption =
		{ "mod-gui.tier-info", (unit_data.conversion_tier or "?"), 17 }
...
script.on_event(defines.events.on_gui_opened, function(event)
	if event.gui_type ~= defines.gui_type.entity or not event.entity or event.entity.name ~= "memory-unit" then
		return
	end
	...
	local main_frame = player.gui.relative.add({
		type = "frame",
		name = "memory_unit_gui",
		caption = { "entity-name.memory-unit" },
		direction = "vertical",
		anchor = {
	...
	local controller_frame = main_flow.add({
		type = "frame",
		name = "content_frame",
		direction = "vertical",
	...
	local controller_header =
		controller_frame.add({ type = "frame", name = "controller_header", style = "subheader_frame" })
	controller_header.style.horizontally_stretchable = true
	controller_header.add({
		type = "label",
		name = "controller_header_label",
		style = "subheader_caption_label",
		caption = { "mod-gui.unit-controller-caption" },
	})
	...
	local entity_preview =
		controller_flow.add({ type = "entity-preview", name = "entity_preview", style = "mu_entity_preview" })
	...
	storage_flow.style.vertical_align = "center"
	local bulk_insert = io_flow.add({
		type = "sprite-button",
		name = "bulk_insert",
		style = "inventory_slot",
		sprite = "bulk-insert",
		tooltip = { "mod-gui.bulk-insert" },
	})
	bulk_insert.tags = { unit_number = entity.unit_number }
	local bulk_extract = io_flow.add({
		type = "sprite-button",
		name = "bulk_extract",
		style = "inventory_slot",
		sprite = "bulk-extract",
		tooltip = { "mod-gui.bulk-extract" },
	})
	...
	local no_input_item = controller_flow.add({
		...
		name = "no_input_item",
		style = "inventory_slot",
		tooltip = { "mod-gui.no-input-item" },
	})
	no_input_item.tags = { unit_number = entity.unit_number }

	local memory_frame =
		main_flow.add({ type = "frame", name = "memory_frame", direction = "vertical", style = "inside_shallow_frame" })
	...
	memory_header_flow.add({
		type = "label",
		name = "memory_header_label",
		style = "subheader_caption_label",
		tooltip = { "mod-gui.memory-tab-tooltip" },
		caption = { "mod-gui.memory-tab-caption" },
	})
	memory_header_flow.add({ type = "label", name = "memory_header_tier_label", style = "subheader_caption_label" })

	local memory_status_flow =
		memory_frame.add({ type = "flow", name = "memory_status_flow", direction = "horizontal" })
	...
	memory_containment_flow.add({ type = "label", name = "memory_containment_label", caption = "Containment field" }).style.right_margin =
		4
	memory_containment_flow.add({
		type = "progressbar",
		name = "memory_containment_strength",
		style = "electric_satisfaction_progressbar",
	}).style.width =
		250

	local mc_frame =
		main_flow.add({ type = "frame", name = "mc_frame", direction = "vertical", style = "inside_shallow_frame" })
	...
	mc_header_flow.add({
		type = "label",
		name = "mc_header_label",
		style = "subheader_caption_label",
		caption = { "mod-gui.matter-tab-caption" },
		tooltip = { "mod-gui.matter-tab-tooltip" },
	})
	...
	mc_info_flow.style.horizontal_align = "right"
	mc_info_flow.style.horizontally_stretchable = true
	mc_info_flow.add({ type = "label", name = "matter_buffer_label", caption = { "mod-gui.matter-tab-bar-caption" } }).style.right_margin =
		4
